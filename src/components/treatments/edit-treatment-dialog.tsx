  ...
  const handleDateSelect = React.useCallback(
    (days: Date[] | undefined) => {
      ...
      currentAppointments.forEach((f, index) => {
        const dateKey = toDate(f.date).toDateString();
        const fieldData = fields[index];
        existingMap.set(dateKey, {
          date: f.date,
          time: f.time || fieldData?.time || DEFAULT_TIME,
          duration: f.duration || fieldData?.duration || DEFAULT_DURATION,
          appointmentId: f.appointmentId || fieldData?.appointmentId,
          status: f.status || fieldData?.status || DEFAULT_STATUS
        });
      });
                        ...
                        fields.map((field, index) => {
                          ...
                          return (
                            <div key={field.id} className="flex flex-wrap items-center gap-3 mb-3 pb-3 border-b last:border-b-0 last:mb-0 last:pb-0">
                              <p className="w-[130px] text-sm font-medium flex-shrink-0">
