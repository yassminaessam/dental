...
const procedurePricingSchema = z.object({
  ...
  patientCopay: z.number().optional(),
});
...
type ProcedurePricing = z.infer<typeof procedurePricingSchema>;

interface AddProviderDialogProps {
  onSave: (data: ProviderFormData & { procedurePricing?: ProcedurePricing[] }) => void;
}
...
export function AddProviderDialog({ onSave }: AddProviderDialogProps) {
  ...
  const onSubmit = (data: ProviderFormData) => {
    onSave({ ...data, procedurePricing: procedurePricing.length > 0 ? procedurePricing : undefined });
    form.reset();
    setProcedurePricing([]);
    setOpen(false);
  };
