/**
 * Direct FTP Upload Test (TypeScript)
 * Tests FTP storage driver directly without needing the dev server
 */

import 'dotenv/config';
import { createFTPDriver } from './src/lib/storage/ftp-driver';
import type { FTPConfig } from './src/lib/storage/ftp-driver';
...
async function testFTPUpload() {
  try {
    console.log('📦 Creating FTP driver...');
    ...
    console.log('✅ FTP driver created\n');
    ...
    protocol.get(result.url, (res) => {
      ...
      if (res.statusCode === 200) {
        ...
        console.log('\n✅ FTP upload successful (redirect is normal)');
      } else {
